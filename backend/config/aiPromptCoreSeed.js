/** Core AI prompts (flash cards, quizzes, TOK, reviews, etc.) — single source for seed + runtime fallbacks. */

export const QUIZ_RATIONALE_SUFFIX = `OUTPUT REQUIREMENT: Your response must be a single valid JSON object with a "questions" array. Every question object MUST include:
...
const CORE_META = {
  ...
  internal_assessment: { category: 'generation', sortOrder: 60 },
  idea_generation: { category: 'ideas', sortOrder: 70 },
  review_internal_assessment: { category: 'review', sortOrder: 80 },
  review_external_assessment: { category: 'review', sortOrder: 90 },
  review_tok: { category: 'review', sortOrder: 100 },
};

/** Full core documents for MongoDB seed and runtime fallbacks. */
export function getCoreAiPromptSpecs() {
  return CORE_AI_PROMPT_SPECS_RAW.map((p) => {
    const meta = CORE_META[p.key] || { category: 'other', sortOrder: 999 };
    return {
      key: p.key,
      name: p.name,
